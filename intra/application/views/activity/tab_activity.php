		...
		foreach ($tab_activity->result() as $value)
		{
			...
			if (!isset($id))
			{
				...
				if (!isset($value->inscrit))
				{
					...
					else if (strtotime($value->date_reg) <= strtotime(date("Y-m-d H:t")) && strtotime($value->date_beg) >= strtotime(date("Y-m-d H:t")))
					{
						if ($value->groupe == 0)
							...
							echo '<td><a href="'.base_url().'activity/register_group/'.$id_mod."/".$activity_name."/".$value->id.'/'.$value->nb_groupe.'"><button class="btn btn-success"  '.$disabled.' style="position:relative;top:0.3em;width:7em;">Group Reg.</button></td>';
