...
class activity extends CI_Controller
{
	...
	public function modify($name_activity, $id_mod, $id_activity)
	{
	...
	public function modify_validation()
	{
		...
		if ($this->form_validation->run())
		{
			...
			$this->modify($activity_name, $id_mod, $data);
	}
	...
	private function deleteScriptCron($id = 0)
	{
	       ...
	       foreach($oldCrontab as $index => $ligne)                  /* copie $oldCrontab dans $newCrontab sans le script à effacer */
	       {
	               if ($isSection == true)                  /*on est dans la section gérée automatiquement*/
	               {
	                       $motsLigne = explode(' ', $ligne);
	                       if ($motsLigne[0] == '#' || $motsLigne[1] == $id)
	                       		$index = $index + 2;
	                       if (($motsLigne[0] != '#' || $motsLigne[1] != $id) && $save_line == 1)        /*ce n est pas le script à effacer*/
	                       {
	                                       $newCrontab[] = $ligne;                 /* copie $oldCrontab, ligne après ligne */
	                       }
	                       else
